...
{
	int x1, y1, x2, y2;
	setlocale(LC_ALL, "Russian"); //русская раскладка 
	cout << "Введите номер вертикали и номер горизонтали первого поля через пробел: ";
	cin >> x1 >> y1; //координаты 1 положения 
	cout << "Введите номер вертикали и номер горизонтали второго поля через пробел: ";
	cin >> x2 >> y2; //координаты 2 положения 
	if ((x1 + y1) % 2 == (x2 + y2) % 2) { //выяснение цвета 
		cout << endl << "Поля одного цвета" << endl << endl;
	}
	else {
		cout << "Поля разных цветов" << endl << endl;
	}
	if (((x1 == x2) or (y1 == y2)) or (abs(x1 - x2) == abs(y1 - y2))) { //проверка угрозы полю ферзём 
		cout << "Ферзь на первом поле угрожает второму полю" << endl << endl;
	}
	else {
		cout << "Ферзь на первом поле не угрожает второму полю" << endl;
	}
	if (((abs(x1 - x2) == 2) and (abs(y1 - y2) == 1)) or ((abs(x1 - x2) == 1) and (abs(y1 - y2) == 2))) { //проверка угрозы полю конём 
		cout << "Конь на первом поле угрожает второму полю" << endl << endl;
	}
	else {
		cout << "Конь на первом поле не угрожает второму полю" << endl << endl;
	}
	if ((x1 == x2) or (y1 == y2)) { //может ли ладья попасть на 2 поле одним ходом
	...
	if ((abs(x1 - x2) == abs(y1 - y2)) or ((x1 == x2) or (y1 == y2))) { // проверка может ли ферзь попасть из первого поля во второе
	...
	if (abs(x1 - x2) == abs(y1 - y2)) { // проверка может ли слон за один ход попасть на 2 поле 
